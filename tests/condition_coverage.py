...
**Borrowed from homegraph, with a hash rather than a sentence.** Source:
`~/homegraph/tests/condition_coverage.py`, sha256
`42cdf00cab41c8f17e30b99847d227c6db870a73e781ebbe720974697301b5b3`, copied
2026-08-09. CP-16 gate 6 diffs this file against that one and allows a
divergence only in the three constants below and in this docstring; anything
else means we forked a tool and called it a loan. Gate 5 re-hashes the source
when it is present and reports SKIPPED, not PASS, when it is not.

**Measured here before the code:** 102 compound conditions in the package, 68
with no mutation aimed at them. That baseline is a ratchet, not a blocker --
see `tests/gold/FASIT-cp16.md` R3.
...
PACKAGE = os.path.join(ROOT, "morpho_homegraph")
...
def changed_lines(since):
    ...
    out = {}
    diff = subprocess.run(
        ["git", "-C", ROOT, "diff", "-U0", since, "--", "morpho_homegraph"],
        capture_output=True, text=True, check=False).stdout
    unstaged = subprocess.run(
        ["git", "-C", ROOT, "diff", "-U0", "--", "morpho_homegraph"],
        capture_output=True, text=True, check=False).stdout
    path = None
    for line in (diff + "\n" + unstaged).splitlines():
...
def main(argv=None) -> int:
    ...
    if args.all:
        ...
        if not scope:
            print("no changed lines under morpho_homegraph/ since %s" % args.since)
            return 0
