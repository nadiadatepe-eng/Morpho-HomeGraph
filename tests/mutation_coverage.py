...
**Borrowed from homegraph**, the stopped predecessor, and registered in the
borrow ledger like `report.py` and `condition_coverage.py`. The source is
`~/homegraph/tests/mutation_coverage.py`, whose sha256 is declared below and
re-hashed by a gate rather than credited in a sentence:

    SOURCE_SHA256 = "ee17784cf847da95237856ba9bf875ea1b1ac3f59eebb2272d5dcdcffedb7373"

**Why it was missing here until 2026-08-20, and what that cost.**
`condition_coverage.py` was carried over on its own and its docstring cites
this file -- so this package shipped a comment pointing at a tool it did not
have. That is the same class of defect `doc-links.py` gates in prose, living
inside a source comment where no gate looked.

The cost was measured, not imagined: CP-23's answer key named twelve gates and
`test_cp23.py` implemented eleven. Gate 11 -- the one that can say *this
checkpoint was not worth building* -- had no check at all. The measurement had
in fact been taken, so nothing was wrong with the result; it was simply
unenforced, and unenforced reads as coverage.

**This tool maps checks to mutations. It does not map answer keys to checks**,
which is the axis that let gate 11 through, so `gate_coverage.py` sits beside
it and asks that second question. Two axes, because a check nobody mutates and
a requirement nobody checks are different holes.
